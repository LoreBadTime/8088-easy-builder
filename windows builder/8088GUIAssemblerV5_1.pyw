...
if os.path.isfile('./dosbox-0.74-3.conf') == True:
    try:
        ...
        print("FATAL:can't move config file,does'" + str("C:/Users/"+ username +"/AppData/Local/DOSBox/' exist?"))
    try:
        os.remove('./dosbox-0.74-3.conf')
    except:
        print("WARN:errors deleting the config file in this folder")
if os.path.isfile('./8088/as88.exe') == True:
    try:
        shutil.copytree('./8088', str("C:/8088"))
    except:
        print("FATAL:errors on install,cant move the 8088 directory into 'C:/' ")
    try:
        shutil.rmtree('./8088')
    except:
        print("WARN:errors on install,cant remove the 8088 directory")
